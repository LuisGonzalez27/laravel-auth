        ...
        <tbody>
            @foreach($projects as $project)
                    <tr>
                        ...
                        <td>{{$project->technologies && count($techologies->technology) > 0 ? count($techology->technologies) : 0}}</td>
                        ...
                        <td> 
                            <form action="{{route('admin.projects.destroy', $project->slug)}}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="delete-button btn btn-danger ms-3" data-item-name="{{$project->name}}"><i class="fa-solid fa-trash-can"></i></button>
                            </form>
                        </td>
                    </tr>
            @endforeach
